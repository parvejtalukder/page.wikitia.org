...
const LoginPage = () => {
    ...
    const signInUser_Func = async (data) => {
        try {
            const res_login = await signInUser(data.email, data.password);
            if (res_login) {
                // await Swal.fire
                await Swal.fire({
                  icon: 'success',
    ...
    <div className="justify-center items-center">
        <form onSubmit={handleSubmit(signInUser_Func)} className="fieldset rounded-box w-xs p-4">
          ...
          <hr />
          {/* <button className="btn btn-neutral mt-4 w-full">Login</button> */}
          <button onClick={loginFunc} className="btn bg-white text-black border-[#e5e5e5]">
          ...
          <label htmlFor="">Don't have account? <span><Link state={location?.state} className='text-blue-600' to={"/register"}>Register</Link></span></label>
        </form>
    </div>
    );
};
